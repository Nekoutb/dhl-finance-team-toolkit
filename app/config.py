...
APP_VERSION = "v11.28.2 — 9 Sep 2026 · Lane focus honours Days to date. With the filter on, the lane's headline AND every customer row cover only the 1st–N of each month — both sides cut the same way, so the comparison is like for like: the month-on-month customer moves, who joined and who left, the concentration shares and the price dispersion all run on the same first-N-days window (shipments are not sliced per day and show as unknown rather than guessed). A month stored before the per-day customer slice existed cannot be cut: Lane focus then shows FULL months and says so — one re-upload, or the 'Re-read the files on record' button, and the filter applies there too."  # lint:country-ok (release note, not behaviour)

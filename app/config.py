...
APP_VERSION = "v11.28.1 — 9 Sep 2026 · Lane focus is a real sandbox now: it sits right after the lanes table with a lane DROPDOWN (top 10 outbound + inbound), shows the lane's RpD w/o fuel surcharge in its headline, and when a month has no per-customer lane detail on record it SAYS so instead of quietly reloading the page. Days to date does the same in the lanes panel: a month stored before per-day lane detail existed now gets a clear notice naming the fix, never a silent wall of dashes. And the fix is one click from now on — each month's IB434 file is RETAINED on upload (one per month), and a 'Re-read the files on record' button re-parses them with the current parser, so future detail additions never again require re-exporting from SAP. Revenue Analysis: Days to date, shipment KPIs, and Lane focus. A DAYS TO DATE filter cuts every comparison — KPI boxes, evolution charts, months side by side, lanes, active customers — to the first N calendar days of each month, so the 1st–10th of this month sits against the 1st–10th of every other. SHIPMENTS and SHIPMENTS/DAY join the compared KPIs everywhere prior periods are compared, and the pricing table carries each customer's shipment count and daily rate. LANE FOCUS: click any lane for five analyses of the customers feeding it, this month vs last — the lane's own KPIs, its top customers month on month, who joined and who left, how concentrated the volume is, and who pays under the lane's going rate. The per-day and per-customer slices behind all this are captured at upload — months already on record need one re-upload to join in."  # lint:country-ok (release note, not behaviour)

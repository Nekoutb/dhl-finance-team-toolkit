...
APP_VERSION = "v11.25 — 27 Aug 2026 · Cash AR stops reading zero, and lanes trend their weight. SAP exports this extract under either the short field names (SAP Acct / Amount / Doc. No.) or the raw technical ones (Customer / Company Code Currency Value / Document Number) — only the first set was recognised, so a file in the other layout read EVERY amount as 0.00 while the open-item counts still looked healthy, and the page filled with dashes. Both layouts now map, French too, and when no amount column is recognised the ageing panel says so instead of quietly showing zeros. The upload itself could also deadlock on its own progress tracker and hang behind the spinner forever, taking the second file down with it — the lock is reentrant now. On Revenue Analysis, every lane's WEIGHT BILLED is trended against its own prior-three-month average beside its RPK: a price per kilo up 50% on volume down 75% is a lane being lost, not a lane being repriced."  # lint:country-ok (release note, not behaviour)
